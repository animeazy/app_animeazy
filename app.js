...
if (path.startsWith("/anime")) {
  ...
  console.group("[AnimePage] boot");
  console.log("animeId:", animeId);
  if (!animeId) { console.warn("→ ?id manquant → showMain()"); showMain(); console.groupEnd(); }
  ...
  console.log("DOM:", { $hero:!!$hero, $logo:!!$logo, $btnLecture:!!$btnLecture, $rowEp:!!$rowEp });
  ...
  async function getAllAnimesCached() {
    ...
    if (c) { console.log("[getAllAnimesCached] cache HIT (len=", c?.length, ")"); return c; }
    console.log("[getAllAnimesCached] cache MISS → fetch catalogue…");
  ...
  async function getAnimeById(animeId) {
    ...
    if (c) { console.log("→ single cache HIT:", { id: c.id ?? c.animeID, titre: c.titre }); console.groupEnd(); return c; }

    try {
      const one = await fetchJson(DETAIL_URL(animeId));
      if (one) { setCache(singleKey, one); console.log("→ DETAIL_URL OK:", { id: one.id ?? one.animeID, titre: one.titre }); console.groupEnd(); return one; }
    } catch (e) { console.warn("→ DETAIL_URL KO:", e.message); }

    console.log("→ fallback catalogue");
    const list = await getAllAnimesCached();
    const found = list.find(a => String(a.id ?? a.animeID) === String(animeId)) || null;
    if (found) { setCache(singleKey, found); console.log("→ found in catalogue:", { id: found.id ?? found.animeID, titre: found.titre }); }
    else { console.warn("→ NOT FOUND in catalogue"); }
    console.groupEnd();
    return found;
  }
  ...
  function extractSeasons(anime) {
    ...
    try {
      if (Array.isArray(anime?.saisons)) {
        ...
        console.warn("shape: inconnue (pas de saisons détectées)");
      }
    } catch(e){ console.warn("extractSeasons error:", e); }
    out.sort((a,b)=> (a.numero||0)-(b.numero||0));
    console.log("→ saisons:", out.map(s => ({ num:s.numero, eps:(s.episodes||[]).length })));
    console.groupEnd();
    return out;
  }
  ...
  function renderHero(anime) {
    ...
    if (poster) {
      const img = new Image();
      img.onload  = () => { console.log("poster preload OK"); setHeroPosterWithOverlay(poster); };
      img.onerror = () => { console.warn("poster preload KO"); setHeroPosterWithOverlay(""); };
      img.src = poster;
    } else {
  ...
  function clearEpisodesRow() {
    ...
    console.log("[Episodes] clearEpisodesRow → nb cards:", cards.length);
    cards.forEach((card, idx) => {
      card.style.display = ""; // visible par défaut
      ...
      card.dataset.epIndex = "";
    });
  }

  function fillEpisodeCard(root, seasonNumber, ep, idx) {
    ...
    // Lien watch
    const epid = ep?.id || ep?.stream_id || `${animeId}-s${seasonNumber||1}e${ep?.numero ?? ep?.num ?? (idx+1)}`;
    ...
    root.dataset.epIndex = String(idx);
    console.log(`  [fill] #${idx+1}`, { title: titleText, epid, img: vignette });
  }

  // Remplit au plus 10 cartes, masque le surplus
  function renderEpisodes(anime) {
    console.group("[Episodes] renderEpisodes");
    if (!$rowEp) { console.warn("row episodes introuvable"); console.groupEnd(); return; }

    const cards = Array.from($rowEp.querySelectorAll(".cards-ep")); // placeholders
    ...
    console.log("episodes détectés (saison", s?.numero, "):", episodes.length,
      episodes.map(e => e?.title || e?.titre).slice(0, 10));
    ...
    for (let i = 0; i < count; i++) {
      ...
      try { fillEpisodeCard(card, s?.numero, episodes[i], i); }
      catch(err) { console.warn(" fillEpisodeCard error @", i, err); }
    }

    for (let i = count; i < cards.length; i++) {
      cards[i].style.display = "none"; // masque les placeholders en trop
      console.log("  [hide] card index:", i, "→ display:none");
    }

    // Vérif finale
    const vis = cards.map((c, i) => ({ i, display: getComputedStyle(c).display }));
    console.log("état final cartes:", vis);
    console.groupEnd();
  }
  ...
  (async function boot() {
    ...
    if (!animeId) { console.groupEnd(); return; }
    ...
    try {
      ...
      console.log("[AnimePage] anime:", { id: anime.id ?? anime.animeID, titre: anime.titre, hasImgLogo: !!anime.url_logo });
      renderHero(anime);
